...
def make_dataset(p: int) -> tuple[torch.Tensor, torch.Tensor]:
    # Input sequence is [a, b, =] where '=' token id is p.
    ...
    x_tensor = torch.tensor(xs, dtype=torch.long)  # [N, 3]
    y_tensor = torch.tensor(ys, dtype=torch.long)  # [N]
...
def _create_runs_dir(experiment: str) -> Path:
    ...
    runs_dir = Path("runs") / "transformer_progress_measures" / experiment / timestamp
...
def main(
    ...
    if cfg.vocab_size != cfg.p + 1:
        raise ValueError(
            f"Expected vocab_size == p + 1 for '=' token, got p={cfg.p}, vocab_size={cfg.vocab_size}"
        )
    ...
    try:
        ...
        plt.ylabel("loss")
        plt.legend()
        plt.title("Progress Measures Loss Curves")
        ...
        plt.ylabel("accuracy")
        plt.legend()
        plt.title("Progress Measures Accuracy Curves")
